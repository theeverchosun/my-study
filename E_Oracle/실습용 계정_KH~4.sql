...
SELECT EMP_NAME, TO_CHAR(TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD' ),'YYYY"년" MM"월" DD"일"')
FROM EMPLOYEE;
...
SELECT EMP_NAME,
       HIRE_DATE,
       TRUNC(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)), ABS(TRUNC(MONTHS_BETWEEN(HIRE_DATE, SYSDATE))) 
FROM EMPLOYEE;
